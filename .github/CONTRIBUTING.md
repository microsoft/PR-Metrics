...
- **PR Metrics** runs the released action for branches hosted in this
  repository.

Pull requests from forks run only read-only validation. Credentialed automation
does not run until a maintainer copies reviewed changes to a branch in this
repository. For branches in this repository, CI commits generated linting,
license, and package changes back to the pull request branch.
