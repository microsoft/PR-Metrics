...
- Target: the open pull request that carries the `release` label (created by
  `Release – Initiate`, with a title beginning `[Autogenerated] Release v`).
...
1. List open pull requests with `list_pull_requests` (state `open`), then filter
   client-side for both the `release` label and a title beginning with
   `[Autogenerated] Release v`. Do not rely on GitHub search syntax for the
   title – the brackets make it unreliable. Confirm exactly one pull request
   matches; if none matches, stop and produce no output.
