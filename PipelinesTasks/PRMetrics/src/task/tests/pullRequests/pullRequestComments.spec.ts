...
//     when(reposInvoker.getComments()).thenResolve([validGitPullRequestCommentThread])
...
//       // Arrange
//       when(reposInvoker.getComments()).thenResolve([{ comments: [{ content: `# Metrics for iteration 10${os.EOL}`, id: 10 }], id: 20 }])
//       const pullRequestComments: PullRequestComments = new PullRequestComments(instance(codeMetrics), instance(inputs), instance(logger), instance(reposInvoker), instance(taskLibWrapper))
...
//       // Arrange
//       when(reposInvoker.getComments()).thenResolve(complexGitPullRequestCommentThreads)
//       when(codeMetrics.getFilesNotRequiringReview()).thenResolve(['folder/file1.ts', 'file2.ts', 'file3.ts'])
//       const pullRequestComments: PullRequestComments = new PullRequestComments(instance(codeMetrics), instance(inputs), instance(logger), instance(reposInvoker), instance(taskLibWrapper))
