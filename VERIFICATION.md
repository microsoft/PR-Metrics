...
1. **Build Provenance Attestation** - Verifiable proof that the artifact was built by GitHub Actions in this repository
2. **Cosign Signature** - Cryptographic signature using Sigstore's keyless signing

Both the main artifact (`ms-omex.PRMetrics.vsix`) and the signature bundle (`ms-omex.PRMetrics.vsix.sigstore.json`) are available on the [GitHub Releases page](https://github.com/microsoft/PR-Metrics/releases).
...
- [GitHub CLI (`gh`)](https://cli.github.com/) version 2.49.0 or later

### Verification Steps

1. Download the VSIX file from the [releases page](https://github.com/microsoft/PR-Metrics/releases):

   ```bash
   gh release download v1.7.10 --repo microsoft/PR-Metrics --pattern "ms-omex.PRMetrics.vsix"
   ```
...
3. Expected output (if verification succeeds):
...
- The artifact was built by the official `release-phase-3.yml` workflow in this repository
...
This returns the attestation bundle in JSON format, which you can verify using standard SLSA verification tools.
...
- [Cosign](https://docs.sigstore.dev/cosign/system_config/installation/) version 2.0 or later
...
1. Download both the VSIX file and the signature bundle:
   ...
   ```bash
   gh release download v1.7.10 --repo microsoft/PR-Metrics --pattern "*.vsix*"
   ```
...
- The signature was created by a workflow running in the `microsoft/PR-Metrics` repository
...
This additionally verifies the specific workflow name that created the signature.

## Security Guarantees

These verification mechanisms provide the following security guarantees to users:
...
1. **Always Verify**: Make verification part of your deployment process, especially in security-sensitive environments

2. **Verify Before Installation**: Run verification checks before installing the VSIX in Azure DevOps or using it in workflows

3. **Automate Verification**: Incorporate verification into your CI/CD pipelines:
...
4. **Use Specific Versions**: Pin to specific release versions rather than using latest

5. **Review Release Notes**: Check the release notes and discussion for each version before deploying
...
- **Malicious Tampering**: If repeated downloads fail verification, report it as a [security issue](SECURITY.md)
...
- [PR Metrics Security Policy](SECURITY.md)
...
2. Review existing [GitHub Discussions](https://github.com/microsoft/PR-Metrics/discussions)
3. Open a new discussion in the [Q&A category](https://github.com/microsoft/PR-Metrics/discussions/new?category=q-a)
