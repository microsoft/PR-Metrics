...
- Comments generated by the task carry hidden markers, and comments are only
  updated or deleted on GitHub when both the marker (or the legacy content) and
  the author match the principal associated with the access token. A PR author
  who copies the markers into their own comment cannot therefore cause the task
