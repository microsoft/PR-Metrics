			...
			testCases.forEach((value: string | null | undefined): void => {
				it(`should throw an error when passed invalid string value '${String(value)}'`, (): void => {
					...
					assert.throws(
						func,
						new TypeError(
							`'string test', accessed within 'string test method name', is invalid, null, or undefined '${String(value)}'.`,
						),
					);
				});
			});
	...
	describe("validateNumber()", (): void => {
		{
			...
			testCases.forEach((value: number | null | undefined): void => {
				it(`should throw an error when passed invalid number value '${String(value)}'`, (): void => {
					// Act
					const func: () => void = () =>
					...
					assert.throws(
						func,
						new TypeError(
							`'number test', accessed within 'number test method name', is invalid, null, or undefined '${String(value)}'.`,
						),
					);
				});
			});
