...
describe('tokenManager.ts', (): void => {
  ...
  beforeEach((): void => {
    ...
    when(azureDevOpsApiWrapper.getHandlerFromToken('AccessToken')).thenReturn(instance(requestHandler))
    ...
    when(runnerInvoker.getInput(deepEqual(['Workload', 'Identity', 'Federation']))).thenReturn('Id')
    when(runnerInvoker.getEndpointAuthorizationScheme('Id')).thenReturn('WorkloadIdentityFederation')
  ...
  describe('getToken()', (): void => {
    it('returns null when no workload identity federation is specified', async (): Promise<void> => {
      ...
      when(runnerInvoker.getInput(deepEqual(['Workload', 'Identity', 'Federation']))).thenReturn(undefined)
    ...
    it('throws an error when the service principal ID is undefined', async (): Promise<void> => {
      // Arrange
      const tokenManager: TokenManager = new TokenManager(instance(azureDevOpsApiWrapper), instance(logger), instance(runnerInvoker))
      when(runnerInvoker.getEndpointAuthorizationParameter('Id', 'serviceprincipalid')).thenReturn(undefined)

      // Act
      const func: () => Promise<string | null> = async () => tokenManager.getToken()

      // Assert
      await AssertExtensions.toThrowAsync(func, '\'servicePrincipalId\', accessed within \'TokenManager.getAccessToken()\', is invalid, null, or undefined \'undefined\'.')
      verify(logger.logDebug('* TokenManager.getToken()')).once()
    ...
    it('throws an error when the tenant ID is undefined', async (): Promise<void> => {
      // Arrange
      const tokenManager: TokenManager = new TokenManager(instance(azureDevOpsApiWrapper), instance(logger), instance(runnerInvoker))
      when(runnerInvoker.getEndpointAuthorizationParameter('Id', 'tenantid')).thenReturn(undefined)

      // Act
      const func: () => Promise<string | null> = async () => tokenManager.getToken()

      // Assert
      await AssertExtensions.toThrowAsync(func, '\'tenantId\', accessed within \'TokenManager.getAccessToken()\', is invalid, null, or undefined \'undefined\'.')
      verify(logger.logDebug('* TokenManager.getToken()')).once()
      verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    })
  ...
  it('throws an error when the endpoint authorization access token is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'endpointAuthorization.parameters.AccessToken\', accessed within \'TokenManager.getSystemAccessToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
    verify(logger.logDebug('Acquired authorization token from workload identity federation.')).once()
  })
  ...
  it('throws an error when the collection URI is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'SYSTEM_COLLECTIONURI\', accessed within \'TokenManager.getFederatedToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
  ...
  it('throws an error when the team project URI is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'SYSTEM_TEAMPROJECTID\', accessed within \'TokenManager.getFederatedToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
  ...
  it('throws an error when the host type is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'SYSTEM_HOSTTYPE\', accessed within \'TokenManager.getFederatedToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
  ...
  it('throws an error when the plan ID is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'SYSTEM_PLANID\', accessed within \'TokenManager.getFederatedToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
  ...
  it('throws an error when the job ID is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'SYSTEM_JOBID\', accessed within \'TokenManager.getFederatedToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
  ...
  it('throws an error when the OIDC token is undefined', async (): Promise<void> => {
    ...
    // Assert
    await AssertExtensions.toThrowAsync(func, '\'response.oidcToken\', accessed within \'TokenManager.getFederatedToken()\', is invalid, null, or undefined \'undefined\'.')
    verify(logger.logDebug('* TokenManager.getToken()')).once()
    verify(logger.logDebug('* TokenManager.getAccessToken()')).once()
    verify(logger.logDebug('* TokenManager.getFederatedToken()')).once()
    verify(logger.logDebug('* TokenManager.getSystemAccessToken()')).once()
