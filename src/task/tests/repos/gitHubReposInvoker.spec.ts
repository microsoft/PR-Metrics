...
import type GetIssueCommentsResponse from '../../src/wrappers/octokitInterfaces/getIssueCommentsResponse'
...
import type OctokitLogObject from '../wrappers/octokitLogObject'
...

    {
      ...
      testCases.forEach(({ message, status }: TestCaseType): void => {
        it('should throw when an error occurs that is not a HTTP 422 or is not due to having a too large path diff', async (): Promise<void> => {
          // Arrange
          const error: TestReposError = new TestReposError(message)
          error.status = status
          when(octokitWrapper.initialize(any())).thenCall((options: OctokitOptions): void => {
            assert.equal(options.auth, 'PAT')
            assert.equal(options.userAgent, expectedUserAgent)
