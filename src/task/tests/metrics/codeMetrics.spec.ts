...
describe('codeMetrics.ts', (): void => {
...

  {
    ...
    testCases.forEach(({ gitResponse, globChecks, metrics, sizeIndicator, testCoverageIndicator }: TestCaseType): void => {
...

  {
    ...
    testCases.forEach(({ deletedFilesNotRequiringReview, filesNotRequiringReview, gitResponse, globChecks, metrics, sizeIndicator, testCoverageIndicator }: TestCaseType): void => {
  ...
  it('with double exclusion ignore patterns ignores the appropriate files', async (): Promise<void> => {
    ...
    when(inputs.growthRate).thenReturn(1.5)
    when(inputs.testFactor).thenReturn(2.0)
    when(inputs.fileMatchingPatterns).thenReturn(['**/*', '!**/ignored*.ts', '!!**/ignored2.ts'])
    when(inputs.codeFileExtensions).thenReturn(new Set<string>(['ts']))
    when(gitInvoker.getDiffSummary()).thenResolve('1\t1\tfile.ts\n1\t1\tignored1.ts\n1\t1\tignored2.ts\n1\t1\tignored3.ts')
...

    {
      ...
      testCases.forEach(({ summary, elements }: TestCaseType): void => {
        it(`should throw when the file name in the Git diff summary '${summary}' cannot be parsed`, async (): Promise<void> => {
          ...
          // Assert
          await AssertExtensions.toThrowAsync(func, `The number of elements '${elements.toString()}' in '${summary.trim()}' in input '${summary.trim()}' did not match the expected 3.`)
          verify(logger.logDebug('* CodeMetrics.getFilesNotRequiringReview()')).once()
          verify(logger.logDebug('* CodeMetrics.initialize()')).once()
          verify(logger.logDebug('* CodeMetrics.createFileMetricsMap()')).once()
        })
