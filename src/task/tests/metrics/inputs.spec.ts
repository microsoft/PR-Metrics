...
describe('inputs.ts', (): void => {
  const adjustingAlwaysCloseComments: string = 'Adjusting the always-close-comments mode input to \'false\'.'
  ...
  const settingAlwaysCloseComments: string = 'Setting the always-close-comments mode input to \'true\'.'
  ...
  beforeEach((): void => {
    ...
    when(runnerInvoker.getInput(deepEqual(['Always', 'Close', 'Comments']))).thenReturn('')
    ...
    when(runnerInvoker.loc('metrics.inputs.adjustingAlwaysCloseComments')).thenReturn(adjustingAlwaysCloseComments)
    ...
    when(runnerInvoker.loc('metrics.inputs.settingAlwaysCloseComments')).thenReturn(settingAlwaysCloseComments)
  ...
  describe('initialize()', (): void => {
    describe('all inputs', (): void => {
      it('should set all default values when nothing is specified', (): void => {
        ...
        expect(inputs.alwaysCloseComments).to.equal(InputsDefault.alwaysCloseComments)
        ...
        verify(logger.logDebug('* Inputs.initializeAlwaysCloseComments()')).once()
        ...
        verify(logger.logDebug('* Inputs.alwaysCloseComments')).once()
        verify(logger.logDebug('* Inputs.fileMatchingPatterns')).once()
        verify(logger.logDebug('* Inputs.codeFileExtensions')).once()
        verify(logger.logInfo(adjustingAlwaysCloseComments)).once()
        ...
        verify(logger.logInfo(settingAlwaysCloseComments)).never()
      ...
      it('should set all input values when all are specified', (): void => {
        ...
        when(runnerInvoker.getInput(deepEqual(['Always', 'Close', 'Comments']))).thenReturn('true')
        ...
        expect(inputs.alwaysCloseComments).to.deep.equal(true)
        ...
        verify(logger.logDebug('* Inputs.fileMatchingPatterns')).once()
        verify(logger.logDebug('* Inputs.codeFileExtensions')).once()
        verify(logger.logInfo(adjustingAlwaysCloseComments)).never()
        ...
        verify(logger.logInfo(disablingTestFactorResource)).never()
        verify(logger.logInfo(settingAlwaysCloseComments)).once()
    ...
    describe('alwaysCloseComments', (): void => {
      {
        const testCases: Array<string | undefined> = [
          undefined,
          '',
        ...
        testCases.forEach((alwaysCloseComments: string | undefined): void => {
          it(`should set the default when the input is '${Converter.toString(alwaysCloseComments)}'`, (): void => {
            // Arrange
            when(runnerInvoker.getInput(deepEqual(['Always', 'Close', 'Comments']))).thenReturn(alwaysCloseComments)
...

      {
        ...
        testCases.forEach((alwaysCloseComments: string): void => {
          it(`should set to true when the input is '${alwaysCloseComments}'`, (): void => {
            // Arrange
            when(runnerInvoker.getInput(deepEqual(['Always', 'Close', 'Comments']))).thenReturn(alwaysCloseComments)
            ...
            // Assert
            expect(inputs.alwaysCloseComments).to.equal(true)
            verify(logger.logDebug('* Inputs.initialize()')).once()
            verify(logger.logDebug('* Inputs.initializeBaseSize()')).once()
            verify(logger.logDebug('* Inputs.initializeGrowthRate()')).once()
            verify(logger.logDebug('* Inputs.initializeTestFactor()')).once()
