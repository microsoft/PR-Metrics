...
describe('inputs.ts', (): void => {
  ...
  const adjustingChangePrTitle: string = 'Adjusting the change-pr-title mode input to \'false\'.'
  ...
  const disablingChangePrTitle: string = 'Setting the change-pr-title mode input to \'true\'.'
  ...
  beforeEach((): void => {
    ...
    when(runnerInvoker.getInput(deepEqual(['Change', 'Pr', 'Title']))).thenReturn('')
    when(runnerInvoker.loc('metrics.inputs.adjustingAlwaysCloseComment')).thenReturn(adjustingAlwaysCloseComment)
    when(runnerInvoker.loc('metrics.inputs.adjustingChangePrTitle')).thenReturn(adjustingChangePrTitle)
    ...
    when(runnerInvoker.loc('metrics.inputs.disablingChangePrTitle')).thenReturn(disablingChangePrTitle)
  ...
  describe('initialize()', (): void => {
    describe('all inputs', (): void => {
      it('should set all default values when nothing is specified', (): void => {
        ...
        assert.equal(inputs.changePrTitle, InputsDefault.changePrTitle)
        ...
        verify(logger.logDebug('* Inputs.initializeChangePrTitle()')).once()
        ...
        verify(logger.logDebug('* Inputs.changePrTitle')).once()
        ...
        verify(logger.logInfo(adjustingChangePrTitle)).once()
        ...
        verify(logger.logInfo(disablingChangePrTitle)).never()
      })
      ...
      it('should set all input values when all are specified', (): void => {
        ...
        when(runnerInvoker.getInput(deepEqual(['Change', 'Pr', 'Title']))).thenReturn('false')
        ...
        assert.deepEqual(inputs.changePrTitle, false)
        verify(logger.logDebug('* Inputs.initialize()')).times(7)
        verify(logger.logDebug('* Inputs.initializeBaseSize()')).once()
        verify(logger.logDebug('* Inputs.initializeGrowthRate()')).once()
        verify(logger.logDebug('* Inputs.initializeTestFactor()')).once()
        ...
        verify(logger.logInfo(disablingTestFactorResource)).never()
        verify(logger.logInfo(adjustingChangePrTitle)).never()
        ...
        verify(logger.logInfo(disablingChangePrTitle)).once()
      })
    })
    ...
    describe('alwaysCloseComment', (): void => {
      {
        ...
        testCases.forEach((alwaysCloseComment: string | undefined): void => {
          it(`should set the default when the input is '${Converter.toString(alwaysCloseComment)}'`, (): void => {
            // Arrange
            when(runnerInvoker.getInput(deepEqual(['Always', 'Close', 'Comment']))).thenReturn(alwaysCloseComment)
