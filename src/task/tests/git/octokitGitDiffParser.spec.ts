...
describe("octokitGitDiffParser.ts", (): void => {
  ...
  beforeEach((): void => {
    httpClient = mock(HttpClientWrapper);
    logger = mock(Logger);
  ...
  describe("getFirstChangedLine()", (): void => {
    ...
    it("should skip a binary renamed file", async (): Promise<void> => {
      // Arrange
      when(octokitWrapper.getPull("owner", "repo", 1)).thenCall(
        async (): Promise<GetPullResponse> =>
          Promise.resolve({
      ...
      when(
        ...
        async (): Promise<string> =>
          Promise.resolve(
            "diff --git a/oldImage.png b/image.png\n" +
              "similarity index 90%\n" +
              "rename from oldImage.png\n" +
              "rename to image.png\n" +
              "Binary files a/oldImage.png and b/image.png differ",
          ),
      );
      const octokitGitDiffParser: OctokitGitDiffParser =
        new OctokitGitDiffParser(instance(httpClient), instance(logger));
      ...
      verify(
        logger.logDebug(
          "Skipping 'RenamedFile' 'image.png' while performing diff parsing.",
        ),
      ).once();
    });
