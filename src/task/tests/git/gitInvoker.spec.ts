...
describe("gitInvoker.ts", (): void => {
  ...
  beforeEach((): void => {
    ...
    when(
      runnerInvoker.exec(
        "git",
        deepEqual(["rev-parse", "--branch", "origin/develop...pull/12345/merge"]),
      ),
    ...
    when(
      runnerInvoker.exec(
        "git",
        deepEqual(["diff", "--numstat", "--ignore-all-space", "origin/develop...pull/12345/merge"]),
      ),
    ).thenCall(
  ...
  describe("isGitRepo()", (): void => {
    {
      ...
      testCases.forEach((response: string): void => {
        it(`should return true when called from a Git repo returning '${response.replace(/\n/gu, "\\n")}'`, async (): Promise<void> => {
          // Arrange
          when(
            runnerInvoker.exec("git", deepEqual(["rev-parse", "--is-inside-work-tree"])),
          ).thenCall(
            async (): Promise<ExecOutput> =>
              Promise.resolve({
                exitCode: 0,
