...
export default class TokenManager {
  ...
  private async getFederatedToken (workloadIdentityFederation: string) : Promise<string> {
    ...
    const hostType: string = validateVariable('SYSTEM_HOSTTYPE', 'TokenManager.getFederatedToken()')
    const planId: string = validateVariable('SYSTEM_PLANID', 'TokenManager.getFederatedToken()')
    const jobId: string = validateVariable('SYSTEM_JOBID', 'TokenManager.getFederatedToken()')
    const response: TaskHubOidcToken = await taskApi.createOidcToken({}, teamProjectId, hostType, planId, jobId, workloadIdentityFederation)

    return validateString(response.oidcToken, 'response.oidcToken', 'TokenManager.getFederatedToken()')
  }
